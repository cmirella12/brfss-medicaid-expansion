...
*** recode data sets from 2014
*** scntmony and scntmeal vars renamed from scntmny1 and scntmel1
*** rename hlthcvr1 to hlthcvrg ** code is different than 2013 


    use "brfss2014.dta", clear 

	keep _state iyear genhlth physhlth menthlth poorhlth hlthpln1 persdoc2 medcost checkup1 exerany2 diabete3 cvdinfr4 cvdcrhd4 cvdstrk3 asthma3 asthnow qlactlm2 smoke100 educa employ income2  sex drnkany5 alcday5 drnk3ge5 maxdrnks emtsuprt lsatisfy qlmentl2 qlstres2 qlhlth2 sleptim1 renthom1 scntmny1 scntmel1 _ststr _strwt _raw _race _racegr3 _rfhlth _hcvu65 marital hlthcvr1 nocov121 _ageg5yr
...
* renaming variables for consistency 
	...
	rename scntmny1 scntmony
	rename scntmel1 scntmeal
	rename hlthcvr1 hlthcvrg
...
* hlthcvrg - Insurance type breakdown
   ...
   // Based on hlthcvr1 = renamed hlthcvrg
generate has_medicaid = hlthcvrg == 4 if !missing(hlthcvrg)
generate has_employer = hlthcvrg == 1 if !missing(hlthcvrg)
generate uninsured_now = hlthcvrg == 8 if !missing(hlthcvrg)
generate other_ins = inlist(hlthcvrg, 2, 3, 5, 6, 7) if !missing(hlthcvrg)

generate ins_type = .
replace ins_type = 1 if has_medicaid == 1
replace ins_type = 2 if has_employer == 1 & missing(ins_type)
replace ins_type = 3 if uninsured_now == 1 & missing(ins_type)
replace ins_type = 4 if other_ins == 1 & missing(ins_type)
