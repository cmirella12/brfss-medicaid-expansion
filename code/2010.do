* 2010 data cleaner *** Recode data sets from 2010 to 2015 
*** recode data sets from 2016-2020 
*** drop variables 


    use "brfss2010.dta" , clear 
	keep _state iyear genhlth physhlth menthlth poorhlth hlthplan persdoc2 medcost checkup1 qlrest2 exerany2 diabete2 lastden3 cvdinfr4 cvdcrhd4 cvdstrk3 asthma2 asthnow qlactlm2 smoke100 age educa employ income2  sex drnkany4 alcday4 drnk3ge5 maxdrnks drnkdri2 emtsuprt lsatisfy qlmentl2 qlstres2 qlhlth2 sleptime adpleasr addown adsleep adenergy adeat1 adthink renthom1 scntmony scntmeal _ststr _strwt _raw _finalwt _region _rfhlth _hcvu65 race2 marital _bmi4cat _ageg5yr 
...
* qlrest2 
   replace qlrest2 = . if qlrest2 == 77 | qlrest2 == 99
   replace qlrest2 = 0 if qlrest2 == 88
   label variable qlrest2 "Days didn't get enough rest/sleep (past 30)"
...
* lastden3 
    replace lastden3 = . if inlist(lastden3, 7, 9)
    label define lastden_lbl ///
     1 "Past year" ///
     2 "Past 2 years" ///
     3 "Past 5 years" ///
     4 "5+ years ago" ///
     8 "Never"
    label values lastden3 lastden_lbl
    label variable lastden3 "Time since last dental visit"
...
* age 
    ...
    label variable age "Reported age in years"
...
* drnkdri2 
    replace drnkdri2 = . if inlist(drnkdri2, 77, 99)
	replace drnkdri2 = 0 if drnkdri2 == 88
    label variable drnkdri2 "Times drove after drinking too much in past 30 days"
	generate drove_drunk = .
    replace drove_drunk = 1 if drnkdri2 > 0
    replace drove_drunk = 0 if drnkdri2 == 0
    label variable drove_drunk "1 = Drove after drinking in past 30 days"
...
* adpleasr
    replace adpleasr = . if inlist(adpleasr, 77, 99)
    replace adpleasr = 0 if adpleasr == 88
    label variable adpleasr "Days had little interest or pleasure (past 2 weeks)"


* addown , adsleep adenergy
    foreach var in addown adsleep adenergy {
      replace `var' = . if inlist(`var', 77, 99)
      replace `var' = 0 if `var' == 88
} 
    label variable addown "Days felt down or hopeless (past 2 weeks)"
    label variable adsleep "Days with sleep issues (past 2 weeks)"
    label variable adenergy "Days with low energy (past 2 weeks)"

* adeat1 
    replace adeat1 = . if inlist(adeat1, 77, 99)
    replace adeat1 = 0 if adeat1 == 88
	label variable adeat1 "Days had poor appetite or ate too much (past 2 weeks)"
    
* adthink
    replace adthink = . if inlist(adthink, 77, 99)
    replace adthink = 0 if adthink == 88
    label variable adthink "Days had trouble concentrating (past 2 weeks)"
...
* _bmi4cat
    replace _bmi4cat = . if _bmi4cat == 9
    label define bmi4cat_lbl ///
     1 "Normal weight or underweight" ///
     2 "Overweight" ///
     3 "Obese"
    label values _bmi4cat bmi4cat_lbl
    label variable _bmi4cat "BMI Category (Normal, Overweight, Obese)"
